(function () {
  ...
  VideoSwitch.prototype.switchOff = function (previousVideo, nextVideo, callback) {
    ...
    function _loop() {
      ...
      var imageWidth = preImageData.width;
      var imageHeight = preImageData.height;
      var nextData = nextImageData.data;
      var pixels = imageWidth * imageHeight;
      for (var x = imageWidth - 1; x >= 0; x--) {
        if (x > endIndex) {
          for (var y = imageHeight - 1; y >= 0; y--) {
            var index = (y * width + x) * 4;  //calculate index 
            data[index] = nextData[index];   // red 
            data[index + 1] = nextData[index + 1]; // green 
            data[index + 2] = nextData[index + 2]; // blue 
  ...
  VideoSwitch.prototype.switchSlide = function () {};
